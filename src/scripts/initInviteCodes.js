...
async function initInviteCodes() {
  try {
    await mongoose.connect(config.mongodb.uri);
    console.log('MongoDB connected');

    for (const code of defaultInviteCodes) {
    ...
    console.log('Invite codes initialization completed');
    process.exit(0);
  } catch (error) {
    console.error('Error initializing invite codes:', error);
    process.exit(1);
  }
}
