...
router.get('/search', auth, async (req, res) => {
  try {
    ...
    if (type) {
      const libraries = await Library.find({ type, owner: req.user.id });
...
router.post('/', auth, async (req, res) => {
  try {
    const { libraryCode, jpName, cnName, code, rarity, condition, versionId } = req.body;

    if (!libraryCode) {
      return res.status(400).json({ error: 'libraryCode is required' });
    }

    const library = await Library.findOne({ code: libraryCode, type: 'local', owner: req.user.id });
    if (!library) {
      return res.status(404).json({ error: 'Local library not found' });
    }

    const count = await Card.countDocuments({ libraryCode });
